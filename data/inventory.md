...
| Class attendance marking | 2 | 1 | 3 | degraded | **Discovered 2026-08-17 by the weekly pulse — nobody had a row for it.** Recording who actually turned up to a class, which is what separates a registration from an attendance and feeds the education KPIs and the post-class follow-up. The purpose-built instructor path — the "Take attendance" screen in `instructor_companion` — is a **hard fatal and has never worked**: `AttendanceForm::buildForm()` calls `$this->entityTypeManager()`, which `FormBase` does not provide, so every instructor who clicks it gets a white screen. Six real attempts in the retained log window. It is not wholly unmarked, because staff can set participant status by hand in CiviCRM: over the last 30 days **12 marked Attended and 2 No-show against 110 still sitting at Registered**, so the fallback runs but rarely. **Fix staged 2026-08-17 (`instructor_companion` 19bae63), deploy pending** — the form now injects the entity type manager it was calling as a helper, verified locally by building and submitting against a real past class. Still `degraded` because it is not on live yet; move to `watch` after deploy and check whether the unmarked-attendance count falls. Scored A2 rather than A3 because the tooling half is dead and the surviving path is a human editing CiviCRM records, and D1 because nothing describes that fallback ⚙ instructor_companion ⧉ P3 |
...
| Badge checkout appointment | 4 | 2 | 4 | stable | A member books a one-to-one slot with a badger to be checked out on a tool and earn the badge. Runs automatically; a fault that broke the 60-minute slot length was fixed in July 2026. ◊ 2026-08-15 ⚠ Staff, 2026-08-14 — part of the badge-earning flow, which staff see as a major driver of retention and a strong candidate for review soon ⧉ S022 |
...
| API endpoint security | 3 | 3 | 5 | broken | Open critical finding: unauthenticated endpoint exposing door and member data ⧉ P2 S037 |
...
| Overdue late fee | 4 | 2 | 4 | degraded | The double-charge fix shipped live 2026-08-14, but the **refunds it was supposed to enable are silently failing**. Two refund attempts on 2026-08-15 (transactions 4851, 4857) were rejected by Stripe: the `rk_live_` restricted key in use lacks `charge_write`, so the site logged an attempt and no money moved. The earlier "refunds pending on 3 members" note therefore reads better than the truth — anyone owed is still out of pocket unless someone refunded by hand in the Stripe dashboard. This is the **second** scope missing from that key found by a production failure (the store tab pilot hit the same key missing `Customers:Write`), so the key's scopes want auditing against every Stripe write path rather than patched one endpoint at a time. Found by the 2026-08-17 weekly pulse ⚙ lending_library,mh_stripe ⧉ S063 S024 |
...
| Store purchase | 3 | 2 | 3 | changing | Member plywood complaint 08-17 confirmed `/store` rendered only classic-PayPal buttons (now behind a PayPal auth wall; sales 168/mo Apr → 28 by mid-Aug) with zero links to the tab/Stripe checkout. Fix staged same day (config-only): every buy button on /store, material pages and related-supplies blocks now routes to `/store/checkout-item/{nid}` with a guest-PayPal fallback. Watching post-deploy: PayPal-vs-Stripe sale split, tab uptake. ◷ 2026-08-17 ⧉ S041 S001* |
...
| Email validation at signup | 1 | 1 | 3 | broken | **Scope measured 2026-08-17 — it was larger than the three hand-found cases suggested.** Across the 1,393 distinct email domains in the CRM, **114 resolve to nothing at all** (no MX, no A, no AAAA), covering **173 contacts, 62 of whom have no Drupal account** — they got a contact record, have heard nothing from us since, and read as no-shows rather than broken signups. A further **120 Drupal accounts** sit on a dead domain. About 25 more domains resolve perfectly well but are one keystroke from a major provider (`gamil.com`, `oulook.com`), which no DNS check can ever catch. The join webform, the guest waiver and CiviCRM's own event registration all accept these today; **CiviCRM registration is the largest source**, and it never passes through a Drupal form. **Fix staged 2026-08-17 (new module `makerspace_email_guard`), deploy pending** — dead domains blocked server-side on the register form, every webform and CiviCRM's forms; live typosquats only ever *suggested*, because `gmx.com`, `aim.com` and `qq.com` are real domains members use and sit just as close to a big provider. Fails open if DNS itself is unreachable, so an outage cannot stop signups. Ships with a read-only report (`drush mh:email-audit`, `/admin/reports/unreachable-email`) for the 62; nothing is auto-corrected, because a confident guess is how a stranger gets mailed someone else's membership details. Still `broken` because none of it is on live; move to `watch` after deploy and re-score A1 → A4 once the guard is actually running ⚙ makerspace_email_guard ⧉ S021 |
| Payment setup | 4 | 2 | 4 | stable | Chargebee portal ⚙ chargebee_portal ⧉ S024 S041 |
| Membership status sync | 4 | 2 | 4 | stable | Webhook to roles and fields ⚙ chargebee_status_sync ⧉ S023 S024 |
| Dunning / payment recovery | 3 | 3 | 4 | stable | Chasing failed and lapsed membership payments before they turn into cancellations. A first phase went live in April 2026. ◊ 2026-08-15 ▦ kpi_payment_resolution_rate,kpi_monthly_revenue_at_risk ⧉ S024 |
...
| Deferred profile field capture | 2 | 1 | 2 | broken | **Re-measured on live 2026-08-17 and it is three separate things, not one.** Discovery is largely recovered — moved to the thank-you interest picker on 2026-08-08, it is filled on 8 of the 15 main profiles created since, against 0 of the preceding 10. Bio now has a capture moment too: the Slack introduction banner writes `field_member_bio` and is no longer dormant (`intro_channel` is set to `#members` on live), but bio is still 0 of 15 — the moment exists and has not yet produced anything. **The third part was never a capture problem at all, it was a lock**: emergency contact name, emergency contact phone and preferred phone were all `required` on `profile.main`, and **115 of 858 active members (13.4%) are missing at least one** — so they could not save *any* profile edit, not a bio, not a headshot, until they supplied an emergency contact nobody had ever asked them for. That is what made every "you can fill this in later" path a dead end, and it was routinely misreported as "the bio field is required" (bio is not required). **Fix staged 2026-08-17 (`profile_membership` c3b2542 plus the matching field config), deploy pending** — the requirement moves to the join path only, so joining members are still asked while established members get a prompt and a one-purpose form at `/membership/emergency-contact` instead of a wall. Verified against a live-cloned database on both paths. Still `broken` because it is not on live; move to `watch` after deploy, and re-measure the 115 after a few weeks — if it does not move, the passive banner is not enough and the next step is an email, not another form tweak ⚙ profile_membership,interests_civi_bridge,slack_member_sync ⧉ S054 |
...
| Process | A | D | I | State | Notes |
|---|---|---|---|---|---|
| Weekly digest | 4 | 3 | 3 | watch | Clean at 886/886. Two staged batches must ship before the next Wednesday send ⚙ makerspace_digest_scheduler ⧉ P8 |
