...
| Storage billing | 3 | 2 | 4 | watch | **Worse than v4 recorded.** A re-audit against live Stripe on 2026-08-14 found **24 active assignments, roughly $565/month uncollected** — not the 7 and ~$225 previously logged. Seven were never linked and seventeen point at dead, cancelled or expired subscriptions. This is ongoing rather than legacy (two failed this week) and the root cause is that nothing feeds subscription state back: there is no webhook or cron reconciliation and the status field was never created. No member has been over-billed. *(JR, round 2)* ◷ 2026-08-14 **Update 2026-08-15:** the feedback loop now exists — `storage_manager` c185556 shipped live with the two status fields that were never created, a weekly reconciliation against the Stripe API, and a digest to staff. Its first read-only run on live reproduced the audit exactly: **$565.00/mo across 24 assignments**, 175 of 255 healthy, of which 11 are automatically repairable and 13 need a person. **Nothing has been charged yet and that is deliberate** — auto-repair ships off so a human reviews the list before any member's card is touched. A moves 2 → 3: detection is now automatic, collection is not. **Update 2026-08-21 — SHIPPED LIVE and the first repair run is done.** The billing-status fix (`storage_manager` e7a6dae) decides status from what Stripe is collecting now, which resolved the eight assignments that *were* being billed but displayed as canceled — Kate reported one of them (Owen Biesel / SS02). Live read-only reproduced exactly eight (IDs 198, 274, 279, 282, 283, 292, 318, 333) and the write run relinked all eight to their real `sub_`/`si_` ids. **No new subscriptions were created** (eight relink notices per run, zero "adopted existing subscription"), and `stripe.reconcile_auto_repair` stayed `false`, so no member's card was touched. The live picture is now 187 healthy of 255, **$332/mo** uncollected — well below the $565 above, because eight of those assignments were never actually unbilled. **Turning auto-repair on is now unblocked and is a deliberate decision, not a leftover**: JR decided 2026-08-20 to enable it after this shipped, and it would charge exactly two people, about $23/mo. Note this could not be rehearsed on test — test resolves a Stripe *test-mode* key, so every live customer id reads as missing and the audit returns meaningless numbers. **2026-08-24 — auto-repair is still off, and turning it on would have billed for a room that no longer exists.** Deleting a storage unit does not close the assignments pointing at it: the assignment survives, still `active`, still priced, pointing at nothing, and the reconciler classified it as *repairable*. Ten units are gone from the table (74, 75, 210, 256–259, 266–268), their labels unrecoverable, and **three active assignments still point at deleted units** — Bob McGonigle $46 (who already pays $433/mo and owes nothing further), Cierra Jade $60 (a real member billed **by invoice**, an exception the system cannot express), and Josephine Ankrah $12 (**no longer a member**; nothing closes storage when membership lapses). Guard staged (`storage_manager` bf2e6f6): a dangling unit is now its own `orphaned_unit` state and is never repairable. **A second, human-factors fault mattered as much as the code one** — the report headed the assignment id `ID` next to a `Unit` column that renders blank for exactly these rows, so the only number on screen read as a unit number and staff acted on that reading; it is now headed `Assignment` and prints `(unit deleted)`. Once the guard deploys, Sasha Camacho (`12A - 4`, $6/mo) is the sole repairable row and auto-repair becomes safe to enable. ⚙ storage_manager ⧉ S041* S024 S082 |
| Store purchase | 3 | 2 | 3 | broken | Member plywood complaint 08-17 confirmed `/store` rendered only classic-PayPal buttons (now behind a PayPal auth wall; sales 168/mo Apr → 28 by mid-Aug) with zero links to the tab/Stripe checkout. Fix shipped live 2026-08-20 (config-only): every buy button on /store, material pages and related-supplies blocks now routes to `/store/checkout-item/{nid}` with a guest-PayPal fallback. **Verified on live 2026-08-20 — the staged config is deployed** (the local-vs-live config diff no longer contains it). **One day of data, too early to call:** in-product transactions ran 43 in July and 57 so far in August, and the day after the deploy carried 6 against a recent daily median of 2. Re-measure the PayPal-vs-Stripe split and tab uptake in the week to 2026-08-27 before moving this to `stable`. **2026-08-24 weekly pulse — the fix itself regressed, and a member found it before we did.** The deployed config builds the guest link as `href="…webscr?{{ nothing_2 }}"`, and `nothing_2` composes its query string with `item_name={{ title }}` — but `title` on this view is the *linked* title field, so its rendered value is a whole `<a href="…">Item Name</a>`. The inner quote closes the `href` early: the guest link points at a truncated, unusable PayPal URL, and the remainder of the query string renders as **visible raw text under every Purchase button** on `/store`, on material pages, and in Related Provided Supplies (four display sites). The member-facing **Purchase** button to `/store/checkout-item/{nid}` is unaffected and still works, so this is `broken` for the guest path and ugly for everyone, not a total outage. It never showed before because `nothing_2` had only ever been consumed through Views' `path:` setting, which URL-encodes it. Reported as feedback sid 20328 on 2026-08-21, the day of the deploy; root-caused in the 2026-08-24 pulse; entry and three fix options in DEPLOY_TODO, public ledger #44550. **The measurement above is now confounded** — any PayPal-vs-Stripe split read in the week to 08-27 is a read of a broken guest link, so re-measure only after the fix. Two related store items came in from the same reporter the same day: material pages reached from `/store` carry no Request Restock button (ledger #44551), and restock-request emails intermittently omit the supplier and material link buttons — a re-report of the same complaint from 2026-05-11 that was never fixed (ledger #44552). ◷ 2026-08-17 ⧉ S041 S001* |
...
| Slack membership lifecycle sync | 4 | 2 | 3 | watch | **Added 2026-08-18 by the module sweep, and it is failing.** The module matches members to Slack by email, stores the Slack user id on the profile, emails a workspace invite to members not yet in Slack, retries on cron until they join or hit the attempt cap, syncs interest channels (#woodshop, #electronics) from stated interests, and — the part that is broken — adds people to the members broadcast channel when the member role is granted and **removes them when it is revoked**. **Fixed 2026-08-21 — and the first successful removal in the module's history happened that evening.** Every removal had failed since install: 24 `restricted_action` rejections on `conversations.kick`. The cause was a Slack workspace permission, not our code — the token (`xoxb`, bot user `makehaven_member_sync`) already carried `channels:manage`, and the target channel `C0AUFPLUR5G` is `#members`, public and not `#general`, so a bot was always permitted in principle. Slack limits removal from public channels to admins by default and a bot can never be an admin. **The control is not on the Permissions page where three separate attempts looked for it** — it lives at `app.slack.com/manage/<team>/permissions/account-types` as the row *"Remove other members from public channels they're a member of"*, and was granted to **Member** on 2026-08-21. That is a deliberate trade: any member can now remove any other member from a public channel. JR accepted it as low-risk (removals are visible in-channel and public channels can be rejoined); the alternative, if it is ever abused, is to run the kick on an admin *user* token and close the permission again. **The 24-person backlog was cleared the same evening**, 22 by permission alone and 2 after repairing their stored Slack id. **Two further problems surfaced while fixing it, and both are larger than the original.** First, `field_member_slack_id_number` holds an email local-part rather than a `U…` id on **1,023 of 2,594 profiles (39%)** — a legacy import, since only **10** of them are current members. Any id-based Slack operation fails `invalid_arguments` for those people; the two we hit were repaired by looking the real id up from Slack by email, which is exactly how the remaining 10 should be fixed. Second, reconciling `#members` against the member role on 2026-08-21 found **971 people in the channel, 809 current members and 160 ex-members** — the module only ever acts on a role-change event, so those 160 lapsed before it shipped and it would never have touched them on its own. **They were drained the same evening: 159 removed, 0 errors, channel 971 → 812, verified by re-listing the channel from Slack rather than trusting the counter.** One of the 160 was deliberately kept — uid 5422 holds the `manager` role and is staff, not a lapsed member, which is the check worth repeating before any future sweep. 183 successful removals are now in the log where there had never been one. **The reason this stays `watch` and not `stable` is that nothing prevents the backlog reforming**: removal still fires only on a role-change event, so any future miss accumulates silently until someone reconciles by hand again. This is the same defect class as storage billing and Chargebee plan sync — an event-driven write with no periodic reconciliation. **That sweep is now built and staged (2026-08-24, `slack_member_sync` e6f3899): `drush slack:reconcile-members` compares the channel to the member role and reports.** It ships inert — cron reporting and cron repair are two separate switches, both off — so the deploy changes nothing until someone decides otherwise. The guards are the substance rather than the sweep: report-only by default, writes refused outside live, protected roles never removed, malformed Slack ids skipped rather than guessed at, unidentified channel members left alone, and an abort that writes **nothing** if proposed removals exceed 25 or 15% of the channel — so that a role query returning nothing becomes a refusal instead of an emptied channel. Writing the tests found two real bugs before deploy, one of which would have made staff removable on the first run. Joining always worked; leaving now works too, but the historical backlog is the real number ⚙ slack_member_sync ⧉ S024 |
