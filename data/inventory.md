...
| Instructor development & peer observation | 1 | 0 | 3 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ EDU O#2 S#2 · DEVELOP |
...
| Class scheduling & publishing | 2 | 1 | 4 | changing | **Corrected from `unknown`.** The education manager engages a contractor who schedules. JR has self-serve scheduling in the pipeline, particularly for repeat instructors ⟐ EDU O#1 S#1 · DEVELOP ‖ [Class Proposal Form](https://www.makehaven.org/Propose-a-class) |
...
| GEMS cohort management | 2 | 1 | 3 | stable | **Corrected from `unknown` — running now**, with details in the event listing ⟐ RET O#3 S#3 · GROW |
...
| Entrepreneurship milestone tracking | 1 | 0 | 3 | planned | Not started. Signup goal data is used for grant reporting and occasional outreach filtering, but no milestone tracking exists. *(JR, round 2)* ⟐ ENT O#1 S#2 · DEVELOP |
| Mentor & advisor matching | 1 | 0 | 3 | planned | Tried and abandoned: "it just did not work well, we would need to do redesign from ground up." A prior attempt exists as evidence, so any restart should begin from why it failed. *(JR, round 2)* ⟐ ENT O#1 S#1 · GROW |
| Entrepreneurship events programming | 2 | 0 | 3 | degraded | **23 participants against a 300 goal.** The gap is 13× ⟐ ENT O#2 S#4 · GROW |
| Cohort programs (Ecolab-style) | 1 | 0 | 3 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ ENT O#2 S#3 · PLAN |
| "Made at MakeHaven" marketplace | 1 | 0 | 2 | stable | Runs at a very light level: a Slack channel where members share design files. Nothing resembling the retail or pop-up presence in the strategic plan. *(JR, round 2)* ⟐ ENT O#3 S#2 · PLAN |
| Ecosystem partner data alignment | 1 | 0 | 2 | planned | Actively being worked on — shared data standards with ClimateHaven, Collab and the Community Foundation. *(JR, round 2)* ⟐ INF O#3 S#4 · GROW |
...
| Accessibility / ADA audit | 2 | 1 | 4 | stable | **Corrected from `undefined`.** A second annual walk-through by the same committee covers accessibility. JR: the audit happens, but there is no defined **review-and-improvement loop** after it ⟐ INF O#2 S#3 · GROW |
...
| Sustainable operations practices | 1 | 0 | 2 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ INF O#1 S#3 · PLAN |
...
| Line-of-business profitability review | 2 | 1 | 4 | stable | **Corrected from `undefined` — the finance committee has actually done this**, breaking results down by line of business. It is laborious, and work is ongoing to make Xero produce it more directly. *(JR, round 2)* ⟐ FIN O#2 S#2 · REFINE |
...
| Rejoin / recapture campaign | 2 | 0 | 3 | stable | Runs: a contractor does outreach and rejoin buttons exist in the product. What has not happened recently is any larger marketing push to lapsed members. *(JR, round 2)* ⟐ ★ RET O#1 S#2 · PLAN |
| Comped / sliding-scale / sponsored memberships | 4 | 1 | 3 | stable | **Corrected from `unknown`.** Applicants self-certify on the join form with an online signature — no approval queue by design ⟐ OUT O#3 S#1 · REFINE ‖ [Scholarship application](https://www.makehaven.org/makehaven-scholarship-application) · [Pricing Policy](https://docs.google.com/document/d/10mGYDuvO_J4AAgWRHIXb3WEO1dU2OQKCnxm2w_0JYpk/edit) |
| Stripe customer linkage | 2 | 1 | 3 | degraded | 656 unlinked; field only written by manual backfills |
| Membership pricing review | 2 | 2 | 4 | stable | **Corrected from `unknown`.** Board policy sets an annual inflation-linked default; the executive director holds the authority to apply it ⟐ FIN O#2 S#1 · GROW ‖ [Pricing Policy](https://docs.google.com/document/d/10mGYDuvO_J4AAgWRHIXb3WEO1dU2OQKCnxm2w_0JYpk/edit) |
...
| Retention intervention tracking | 3 | 1 | 3 | stable | A contractor works the at-risk queue using member-success module data. Whether interventions actually change outcomes is charted but not, as far as we can tell, acted on. *(JR, round 2)* ⟐ RET O#1 S#1 · DEVELOP |
...
| Interest-based member groups | 1 | 0 | 2 | stable | Runs loosely through Slack channels and meetups rather than as a supported programme with stipends. *(JR, round 2)* ⟐ RET O#3 S#1 · GROW |
| Member recognition & awards | 1 | 0 | 2 | changing | In the works, and has been for a long time — the awards concept keeps being picked up and put down. *(JR, round 2)* ⟐ RET O#3 S#2 · GROW |
...
| Community Wishlist | 4 | 1 | 2 | stable | **Corrected from `undefined` — it exists and runs.** Members submit and vote; it also feeds the tool-acquisition process as the step after budget ⟐ RET O#4 S#1 · DEVELOP ‖ [Wishes](https://www.makehaven.org/wishes) |
...
| Quarterly close-the-loop reporting | 1 | 0 | 3 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ RET O#4 S#2 · GROW |
...
| Workshop participant → member conversion | 2 | 0 | 4 | degraded | JR: it *should* send an email and re-invite attendees, but that needs verifying and is undeveloped either way. 6.4% conversion against a 10% goal, and the strategic plan carries its own note that this is not properly measured. *(JR, round 2)* ⟐ OUT O#1 S#1 · PLAN |
...
| Referral / bring-a-friend | 2 | 1 | 3 | degraded | **Corrected from `unknown` — it runs, but by hand.** Members do refer; JR names it explicitly as an area for technical process improvement ⟐ OUT O#2 S#1 · GROW ‖ [Referral program](https://www.makehaven.org/member-referral-program) |
| Ambassador program | 1 | 0 | 3 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ OUT O#2 S#1 · GROW |
| Community tabling & partner events | 2 | 0 | 2 | stable | Confirmed running. *(JR, round 2)* ⟐ OUT O#2 S#3 · GROW |
| Community partnership development | 2 | 0 | 3 | stable | Confirmed running, though without a defined pipeline or count. *(JR, round 2)* ⟐ OUT O#3 S#2 · PLAN |
| Media relations & press kit | 1 | 0 | 2 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ OUT O#2 S#4 · PLAN |
| Neighborhood & postcard campaigns | 2 | 1 | 2 | stable | Run as experiments rather than on a schedule — planned ad hoc rather than triggered. *(JR, round 2)* ⟐ ★ OUT O#2 S#2 · PLAN |
...
| Monthly financial close | 2 | 2 | 4 | stable | **Corrected from `unknown`.** A contractor does data entry, the executive director handles the harder work, finance committee volunteers assist, and a CPA reviews and prepares the 990 ⟐ FIN O#3 S#1 · REFINE |
...
| Reserve fund management | 2 | 2 | 5 | stable | **Corrected from `unknown`.** Governed by published financial and procurement policies, managed by the executive director. Reserve stands at 2.93 months against a 6-month goal ⟐ ★ FIN O#1 S#1 · DEVELOP ‖ [Financial policies](https://www.makehaven.org/makehaven-financial-policies-and-procedures) · [Procurement policy](https://www.makehaven.org/makehaven-procurement-policy) |
| Annual external review / audit | 2 | 2 | 4 | stable | **Corrected from `unknown`.** Completed last year; this year's documents are already with the reviewer ⟐ FIN O#3 S#3 · DEVELOP ‖ [Filing requirements & procedures](https://docs.google.com/document/d/1utOZLoGWzUXXe0hS3D7jt1YPEMKZ93yD1ztQedEaF0k/edit) |
| Financial policy maintenance | 2 | 2 | 3 | stable | **Corrected from `undefined`** — both policies are published and current ⟐ FIN O#3 S#3 · DEVELOP ‖ [Financial policies](https://www.makehaven.org/makehaven-financial-policies-and-procedures) · [Procurement policy](https://www.makehaven.org/makehaven-procurement-policy) |
...
| Process | A | D | I | State | Notes |
|---|---|---|---|---|---|
| Annual appeals (spring + year-end) | 2 | 1 | 4 | stable | **Corrected from `unknown`.** Run by the Resource Development committee with Kate as staff lead ⟐ DEV O#2 S#1 · MAINTAIN ‖ [Fundraising Plan](https://docs.google.com/document/d/1ehYHyPDqcll_xW0tpcqhJt237GxUi7VjD3-Gvqd5BBo/edit) |
| Recurring giving program | 2 | 1 | 3 | stable | Owned by Kate with the resource development committee; 39 recurring donors against a 60 goal. *(JR, round 2)* ⟐ DEV O#2 S#2 · REFINE |
...
| Major donor stewardship cadence | 2 | 1 | 4 | stable | **Corrected from `undefined`.** Staff steward major donors and the board is sometimes involved. JR: the system could be built out — cadence is not yet systematic ⟐ DEV O#3 S#2 · PLAN |
...
| Sponsor recognition & benefit fulfilment | 2 | 1 | 3 | stable | Kate leads, shared with the executive director for sponsor relationships. *(JR, round 2)* ⟐ DEV O#1 S#2 · DEVELOP |
...
| Donor data hygiene in CRM | 2 | 1 | 3 | stable | **Kate owns donor data quality** and does most of the checking, shared with the executive director for major donors and sponsors. *(JR, round 2)* ⟐ DEV O#5 S#1 · DEVELOP |
| Case statement maintenance | 1 | 0 | 2 | planned | Confirmed by JR as an intention we have not started — a roadmap item, not a broken process. *(JR, round 2)* ⟐ DEV O#4 S#3 · PLAN |
| Planned giving / bequest program | 1 | 0 | 2 | degraded | A page exists but there is no cultivation activity behind it — present in form, dormant in practice. *(JR, round 2)* ⟐ DEV O#5 S#4 · PLAN |
