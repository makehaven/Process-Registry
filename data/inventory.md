...
| Deferred profile field capture | 2 | 1 | 2 | broken | **Re-measured on live 2026-08-17 and now three separate things, not one.** Discovery is largely recovered — moved to the thank-you interest picker on 2026-08-08, it is filled on 8 of the 15 main profiles created since, against 0 of the preceding 10. Bio has finally acquired a capture moment too: the Slack introduction banner writes `field_member_bio` and is **no longer dormant** (`intro_channel` is set to `#members` on live), but bio is still 0 of 15, so the moment exists and has not yet produced anything. **The part that has not moved is a lock, not a capture problem**: emergency contact name, emergency contact phone and preferred phone are all required on `profile.main`, and **114 of 858 active members (13.3%) are missing at least one** — so they cannot save *any* profile edit until they supply an emergency contact they were never asked for, which is what makes every "you can fill this in later" path a dead end. That is a config decision rather than a build ⚙ profile_membership,interests_civi_bridge,slack_member_sync ⧉ S054 |
